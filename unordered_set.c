...
static void resize_set(UnorderedSet **a_set);
static void delete_GraphNode(GraphNode **a_nd);
static void delete_List(GraphNode **a_head);
...
int UnorderedSet_insert(UnorderedSet **a_set, char *key)
{
    ...
    if (nd == NULL)
    {
        ...
        if (nd != NULL)
        {
            ...
            *nd = (GraphNode){
                .key = malloc(sizeof(*(nd->key)) * ((*a_set)->key_len + 1)),
                .in_degree = 0,
                .in_edges = {0, 0, 0, 0},
                .out_degree = 0,
                .out_edges = {0, 0, 0, 0},
...
static void resize_set(UnorderedSet **a_set)
{
    ...
    size_t new_size = (*a_set)->num_buckets * GROWTH_MULTIPLIER;

    // check for overflow
    if (new_size <= (*a_set)->num_buckets)
    {
        return;
    }
    //printf("new graph size = %ld\n", new_size); 

    UnorderedSet *new_set = init_UnorderedSet(new_size, (*a_set)->key_len);
    if (new_set != NULL)
    {
        GraphNode *temp = NULL;
        GraphNode *next = NULL;
